...
public class Game {
	...
	private void step(GameGrid grid) {
		...
		{
			for(int j = 0; j < grid.getGridCellContents().get(i).getCellNeighbours().size(); j++)
			{
				gameRules.deadWithThreeLivingNeighboursSpawn(grid.getGridCellContents().get(i));
				gameRules.tooManyOrTooLittleNeighbours(grid.getGridCellContents().get(i));
			}
		}
		...
		for(int i = 0; i < grid.getTotalGridSize(); i++)
		{
			if(grid.getGridCellContents().get(i).isSwitchState()){
				grid.getGridCellContents().get(i).setStateAlive(!grid.getGridCellContents().get(i).isStateAlive());
	...
	private void displayGrid(GameGrid grid) {
		...
		for(int i = 0; i < grid.getTotalGridSize(); i++)
		{
			...
			{
				System.out.print(grid.getCharacterForCell(grid.getGridCellContents().get(i)) + "    ");
				edgeCount++;
			}
